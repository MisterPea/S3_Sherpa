...
describe('List Item Folder Component', () => {
  test('renders list item with correct label', () => {
    const { container } = render(<ListObject mainAction={() => { }} secondaryAction={() => { }} label="List Item Object" menuDisabled={false} />);
  ...
  test('renders correct storage class', () => {
    const { container } = render(<ListObject mainAction={() => { }} secondaryAction={() => { }} label="List Item Object" storageClass="Standard" menuDisabled={false} />);
    const labelElement = container.querySelector('.list_object_folder-storage_class');
    expect(labelElement).toHaveTextContent('STANDARD');
  });

  test('renders element with appropriate icons', () => {
    render(<ListObject mainAction={() => { }} secondaryAction={() => { }} label="List Item Object" menuDisabled={false} />);
  ...
  test('calls action when main area clicked', () => {
    ...
    const { container } = render(<ListObject mainAction={mainMockAction} secondaryAction={secondaryMockAction} label="List Item Object" menuDisabled={false} />);
    const mainButton = container.querySelector('.list_object_folder-button');
    const secondaryButton = container.querySelector('.context_button_ellipsis');
  ...
  test('secondary action should not be called when clicked and menu is disabled', () => {
    const mainMockAction = jest.fn();
    const secondaryMockAction = jest.fn();
    const { container } = render(<ListObject mainAction={mainMockAction} secondaryAction={secondaryMockAction} label="List Item Object" menuDisabled />);
    const secondaryButton = container.querySelector('.context_button_ellipsis');
    fireEvent.click(secondaryButton!);
    expect(secondaryMockAction).not.toHaveBeenCalledTimes(1);
  });
